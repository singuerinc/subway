        ...
        for (let i = 0; i < data.length; i += 1) {
            ...
            if (this.stations.has(info.id)) {
                ...
                if (prevStation) {
                    ...
                    const distanceBtwStations = MathUtils.distance(station.position.x, station.position.y, px, py);
                    const numWayPoints = Math.floor(distanceBtwStations / 40);
                    for (let j = 0; j < numWayPoints - 1; j += 1) {
                        const percentage = (1 / numWayPoints) * (j + 1);
                        const [x, y] = MathUtils.midpoint(px, py, station.position.x, station.position.y, percentage);
